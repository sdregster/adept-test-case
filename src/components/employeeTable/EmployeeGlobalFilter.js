...
export const EmployeeGlobalFilter = ({ filter, setFilter, rowsArray }) => {
    ...
    const addEmployeeModal = useSelector(state => state.modal.addEmployeeModal)
    const addEmployeeModalHandler = (boolean) => {
        dispatch(setAddEmployeeModalStatus(!boolean))
    }

    const currentCompany = useSelector(state => state.employee.employees)
    const removeHandler = () => {
        const removeData = {
            id: currentCompany.id,
            arrayIds: selectedRows
        }
        dispatch(removeEmployee(removeData))
    }
    ...
    return (
        <span className='flex gap-1 mb-1'>
            ...
            />
            <button
                onClick={() => addEmployeeModalHandler(addEmployeeModal)}
